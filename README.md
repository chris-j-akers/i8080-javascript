...
Unit tests cover nearly all the 8080 operations. They're generated by the `test_generator.py` application found in `/utils` which reads in a set of `YAML` config files to generate *Mocha* test suites. This simple application saves a lot of time in maintaining unit tests that contain very similar boilerplate code but with different inputs and different results.

The `YAML` files should be quite simple to work out. The top section forms variables for the whole test suite such as any boilerplate code and the bottom section details the tests and the expected results.
