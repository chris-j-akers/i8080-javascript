...
* [src/core/](src/core/)  
   ...
   An Intel 8080 CPU emulator written in JavaScript, and components that can be used to build a virtual machine using the Intel 8080 as the CPU.
...
* [documentation](documentation)
  ...
  Documents about the design of the app and third-party documentation that was used during development.

* [README](README.md)

  This README made up of notes regarding the implementation of the emulator. For details, please consult the code or try just asking :-)
...
The `YAM:` files should be quite simple to work out. The top section forms variables for the whole test suite such as any boilerplate code and the bottom section details the tests and the expected results.
...
3. Call the `MVI` command to load immediate value `0x76` (the HALT opcode) into the 16bit address now loaded into the `H` and `L` registers (`0x26FF`). This ensures that, if the code does jump to this location, the program will end.
