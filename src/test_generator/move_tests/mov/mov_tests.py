...
def generate_mov_reg_to_reg(boilerplate, test):
    return boilerplate.format(reg_source = test['reg_source'], 
                                ...
                                mov_opcode=hex(test['mov_opcode']))
...
def generate_mov_mem_to_reg(boilerplate, test):
    return boilerplate.format(reg_destination=test['reg_destination'], max_mem_addr=config.MAX_MEM_ADDR)

def generate_mov_reg_to_mem(boilerplate, test):
    return boilerplate.format(reg_source = test['reg_source'], max_mem_addr=config.MAX_MEM_ADDR)
